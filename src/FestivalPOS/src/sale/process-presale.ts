...
export class ProcessPresale {
  ...
  get totalQuantity() {
    if (this.order) {
      return this.order.lines.reduce(
        (sum, line) => sum + (line.redeem ? line.receiveable : 0),
        0,
      );
    }
  }
  ...
  async activate(params: { orderId: string; tagNumber?: string }) {
    ...
    for (const line of this.order.lines) {
      line.canRedeem = !!(
        line.receiveable &&
        line.productId &&
        products.find((x) => x.product.id === line.productId && !x.presale)
      );
  ...
  async process() {
    ...
    try {
      ...
      const serving = await this.api
        .createServing(this.order.id, {
