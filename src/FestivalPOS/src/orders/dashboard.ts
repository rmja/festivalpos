...
export class OrdersDashboard {
    ...
    async deleteHistory() {
        let answer = prompt(`Skriv startdatoen hvorfra alle ordre slettes i formatet 'yyyy-mm-dd'.`);
        if (!answer) {
            return;
        }
        ...
        if (prompt(`Skal alle ordre siden d. ${date} slettes? Skriv 'JA' for at bekræfte`) === "JA") {
