...
export function ensureValidState(state: State) {
  ...
  newState.orderLines = state.orderLines.map((x) =>
    Object.assign(x, {
      total: new Big(x.total),
    }),
  );
  newState.currentMiscOrderLine.total = new Big(
    newState.currentMiscOrderLine.total,
  );
...
export function setup(
  ...
  newState.terminalId = options.terminalId;
...
export function addProductOrderLine(
  state: State,
  product: { id: number; name: string; price: Big },
  presale: boolean,
  isServing: boolean,
) {
  const newState = Object.assign({}, state);
...
export function getOrderTotal(state: State) {
  return state.orderLines.reduce(
    (current, line) => current.plus(line.total),
    state.currentMiscOrderLine.total,
  );
}
...
export function getOrderQuantityTotal(state: State) {
  return state.orderLines.reduce(
    ...
    state.currentMiscOrderLine.total.eq(0) ? 0 : 1,
  );
}
