...
export class ServingDashboard {
  ...
  async activate(params: { beam?: true }) {
    ...
    const [pos, servings] = await Promise.all([
      ...
      this.api
        .getAllServingsByPointOfSaleId(this.state.pointOfSaleId)
        .bypassClientCache()
        .transfer(),
    ]);
    this.servingStaff = pos.servingStaff.map(
      (x) => new ServingStaffViewModel(this, x),
    );
    ...
    this.helloHandle = window.setInterval(
      () => this.hub.hello(this.state.pointOfSaleId),
      10_000,
    );

    this.disposables = [
      this.eventAggregator.subscribe(ServingCreated, (event: ServingCreated) =>
        this.addOrUpdateServing(event.serving),
      ),
      this.eventAggregator.subscribe(ServingUpdated, (event: ServingUpdated) =>
        this.addOrUpdateServing(event.serving),
      ),
      this.eventAggregator.subscribe(
        PointOfSaleUpdated,
        (event: PointOfSaleUpdated) =>
          (this.servingStaff = event.pos.servingStaff.map(
            (x) => new ServingStaffViewModel(this, x),
          )),
      ),
    ];
  ...
  getViewStrategy() {
    return this.beam
      ? PLATFORM.moduleName("./dashboard-beam.html")
      : PLATFORM.moduleName("./dashboard.html");
  }
  ...
  async addServer() {
    ...
    if (!result.wasCancelled) {
      ...
      await this.api
        .updatePointOfSale(this.state.pointOfSaleId, [
  ...
  async editServer(staff: ServingStaffViewModel, index: number) {
    const result = await this.dialog
      .open({ viewModel: ServerDialog, model: { name: staff.name } })
      .whenClosed();
  ...
  async acceptNextServing(staffNumber: number) {
    ...
    const serving =
      ...
      pendingServings.find((x) => x.highPriority) ||
      pendingServings[0];
    this.selectedServing = undefined;
  ...
  private addOrUpdateServing(serving: ServingViewModel) {
    this.logger.info(
      `Handling serving ${serving.id} created ${serving.created}`,
    );
...
class ServingStaffViewModel {
  ...
  get ongoingServings() {
    return this.dashboard.servings.filter(
      (x) => x.staffNumber === this.number && x.state === "ongoing",
    );
  }

  get completedServings() {
    return this.dashboard.servings.filter(
      (x) => x.staffNumber === this.number && x.state === "completed",
    );
  }

  constructor(
    private dashboard: ServingDashboard,
    staff: ServingStaff,
  ) {
